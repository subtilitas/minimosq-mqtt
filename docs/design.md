...
| Payload larger than `max_payload_len` | delivered QoS 0 pass-through only; QoS>0 subscribers skipped and `delivery_dropped` reported (size `max_payload_len >= max_packet_size` to avoid; `==` suffices for client publishes, whose payload cannot exceed the body carrying it, while more raises the ceiling for `Broker::publish()`) |
